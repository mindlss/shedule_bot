...
import { getScheduleForDate, getScheduleForWeek } from './getSchedule.js';
import { generateDay } from './generateImage.js';
...
schedule.scheduleJob({ hour: 17, minute: 0, dayOfWeek: [1, 2, 3, 4, 5, 7] }, async () => {
    ...
    await generateDay(data.dateInfo, data.lessons, data.location, data.noPairs, './auto_schedule.png');
...
async function test() {
    ...
    const shed = await getScheduleForWeek("2025-02-03")
